...
def check_file(app_fname, md5_url):
    '如果文件MD5值与网上提供的一致， 返回True；否则返回False'
    m = hashlib.md5()
...
if __name__ == '__main__':
    ...
    ver_url = 'http://192.168.1.12/deploy/live_ver'
    # 判断是否有新版本，没有则退出
    ...
    r = requests.get(ver_url)
    app_url = 'http://192.168.1.12/deploy/pkgs/meweb-%s.tar.gz' % r.text
    wget.download(app_url, down_dir)

    # 检验软件包是否损坏，如损坏则删除它并退出
